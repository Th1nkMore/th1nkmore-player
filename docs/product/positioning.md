...
The current implementation provides the player, lyrics, gesture-aware responsive navigation, ordinary uploads, verified external-cover package review, editing, Creator Note authoring, and deployment. Full-cover production now belongs to HuangToolbar and Audacity; the website retains only the separate lightweight Creator Note recorder. The information story experience and one-audio-focus behavior are implemented locally; track share pages, playback persistence, and per-track visual themes remain roadmap work.
